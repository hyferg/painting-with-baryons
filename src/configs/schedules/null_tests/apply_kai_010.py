...
from src.features.transforms import create_range_compress_transforms
...
range_compress_transform, range_compress_inv_transform = \
 create_range_compress_transforms(k_values={"dm": 1.5, "pressure": 1.35},
                                  modes={'dm': 'x/(1+x)',
                                         'pressure': 'log'},
                                  scale=1.75/2,
                                  delta=(2-1.75)/2)
...
schedule['save_summary']['iters'] = [10, 25] + np.arange(0, 350, 50).tolist() + np.arange(350, 10000, 100).tolist()

schedule['loss_params']['l1_lambda'] = 2e5
